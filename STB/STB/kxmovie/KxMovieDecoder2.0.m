...
- (kxMovieError) openInput: (NSString *) path
{
    ...
    if (_interruptCallback) {
        ...
        formatCtx->max_analyze_duration=7000000;
...
- (kxMovieError) openVideoStream
{
    ...
    for (NSNumber *n in _videoStreams) {
         ...
         NSUInteger iStream = n.integerValue;
//        if (n.integerValue > 3) {
//            iStream = n.integerValue +2;
//        }
        iStream =  av_find_best_stream(_formatCtx,AVMEDIA_TYPE_VIDEO,-1,-1,NULL,0);
        relatedvideo = iStream;
        NSLog(@"istream == ::%d",iStream);
...
{    
    ...
    codecCtx->error_concealment = FF_EC_DEBLOCK;
    codecCtx->err_recognition = AV_EF_AGGRESSIVE;
...
//            iStream = n.integerValue +2;
//        }
        iStream =  av_find_best_stream(_formatCtx,AVMEDIA_TYPE_AUDIO,-1,relatedvideo,NULL,0);
        NSLog(@"istream audio== ::%d",iStream);
        errCode = [self openAudioStream: iStream];
        if (errCode == kxMovieErrorNone)
            break;
    }    
    return errCode;
        ...
        if (packet.stream_index ==_videoStream) {
            ...
            while (pktSize > 0) {
                ...
                if (gotframe) {
                    ...
                    if (frame) {
                        AVCodecContext *h = _videoFrame->owner;
                        if (h->flags2 == 4) {
                            NSLog(@"decode mosaic, skip packet");
                            counter=50;
                        }
                        ...
                        if (decodedDuration > minDuration)
                            finished = YES;
                        if (counter>0) {
                            NSLog(@"come into counter%u",counter);
                            [result removeLastObject];
                            counter--;
                            finished = NO;
                        }
