...
//sly
static CGFloat const channelWidth = 100;
static CGFloat const advertisementHeight = 40;
...
//sly
@property (readonly,nonatomic) UITableView *tblChannel;
...
#pragma mark------------界面初始化
//节目列表
- (UITableView*)tblChannel
{
    CGRect bounds = [[UIScreen mainScreen] applicationFrame];
    static VCManager *tblChannel = nil;
    if (tblChannel==nil) {
        tblChannel = [[VCManager alloc] initWithFrame:CGRectMake(0, 0, channelWidth, bounds.size.width-advertisementHeight) style:UITableViewStylePlain];
//        tblChannel.autoresizingMask = UIViewAutoresizingFlexibleTopMargin | UIViewAutoresizingFlexibleBottomMargin | UIViewAutoresizingFlexibleLeftMargin |UIViewAutoresizingFlexibleHeight;
        tblChannel.showsHorizontalScrollIndicator = NO;
        tblChannel.showsVerticalScrollIndicator = NO;
        tblChannel.dataSource = tblChannel;
        tblChannel.delegate = tblChannel;
    }
...
//    [self.view addSubview:_topHUD];
    ...
    [self.view addSubview:self.tblChannel];
}
...
//    [self dealloc];
    ...
    NSString *path = [[NSBundle mainBundle] pathForResource:@"Carwash Orgy" ofType:@"rmvb"];
    path  = @"http://192.168.1.3:8080/files/player.mosaic";
...
- (void) setupPresentView
{
    ...
    bounds.origin.x = channelWidth;
    bounds.size.width -=channelWidth;
    if (_decoder.validVideo) {
...
- (void)showChannel: (BOOL) show
{
    CGFloat X = _hiddenHUD ? channelWidth*-1 : 0;
    CGRect tblChannelFrame = self.tblChannel.frame;
    tblChannelFrame.origin.x = X;
    self.tblChannel.frame = tblChannelFrame;
}
