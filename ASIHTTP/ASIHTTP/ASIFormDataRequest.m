...
+ (id)jsonRequestWithURL:(NSURL *)newURL
{
	return [[[self alloc] initJsonRequestWithURL:newURL] autorelease];
}
...
- (id)initJsonRequestWithURL:(NSURL *)newURL
{
	self = [super initWithURL:newURL];
	[self setPostFormat:ASIJsonPostFormat];
	[self setStringEncoding:NSUTF8StringEncoding];
    [self setRequestMethod:@"POST"];
	return self;
}
...
- (void)buildPostBody
{
    ...
    switch (self.postFormat) {
        case ASIURLEncodedPostFormat:
            [self buildURLEncodedPostBody];
            break;
        case ASIMultipartFormDataPostFormat:
            [self buildMultipartFormDataPostBody];
            break;
        case ASIJsonPostFormat:
...
- (void)buildJsonPostBody
{
	...
	for (NSDictionary *val in [self postData]) {
        NSString *pKey = [val objectForKey:@"key"];
        NSString *kValue = [val objectForKey:@"value"];
        jsonDic[pKey] = kValue;
	}
    ...
    if (jsonError!=nil) {
        NSLog(@"file:%s line:%d,error:%@",__FILE__,__LINE__,jsonError);
    }
    
    [super appendPostData:jsonData];
}
