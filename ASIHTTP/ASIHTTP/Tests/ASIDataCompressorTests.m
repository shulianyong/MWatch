...
- (void)testInflateData
{
	...
	XCTAssertTrue(success,@"inflated data is not the same as original");
...
- (void)testDeflateData
{
	...
	success = ([inflatedString isEqualToString:originalString]);
	XCTAssertTrue(success,@"deflate data is not the same as that generated by gzip");
	...
	XCTAssertTrue(success,@"inflated data is not the same size as the original");
