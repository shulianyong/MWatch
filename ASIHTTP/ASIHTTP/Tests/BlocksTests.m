...
- (void)testBlocks
{
	...
	XCTAssertFalse(failed,@"Request failed, cannot proceed with test");
	XCTAssertTrue(started,@"Failed to call started block");
	XCTAssertTrue(receivedHeaders,@"Failed to call received headers block");
	XCTAssertTrue(complete,@"Failed to call completed block");
	
	BOOL success = (totalBytesReceived == 457);
	XCTAssertTrue(success,@"Failed to call bytes received block, or got wrong amount of data");
	success = (totalDownloadSize == 457);
	XCTAssertTrue(success,@"Failed to call download size increment block");
	
	success = (totalBytesSent == [dataToSend length]);
	XCTAssertTrue(success,@"Failed to call bytes sent block");
	success = (totalUploadSize == [dataToSend length]);
	XCTAssertTrue(success,@"Failed to call upload size increment block");
	...
	[request startSynchronous];
	XCTAssertTrue(failed,@"Failed to call request failure block");
}
#endif
