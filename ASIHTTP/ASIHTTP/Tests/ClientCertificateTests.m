...
- (void)testClientCertificate
{
	...
	XCTAssertNotNil([request error],@"Request succeeded even though we presented no certificate, cannot proceed with test");
	...
	XCTAssertNil([request error],@"Request failed with error %@",[request error]);
	BOOL success = [[request responseString] isEqualToString:@"This is the expected content for the first string"];
	XCTAssertTrue(success,@"Request failed to download the correct content");
}
