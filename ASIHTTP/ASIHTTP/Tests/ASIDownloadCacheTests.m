...
- (void)testDownloadCache
{
	...
	XCTAssertTrue(success,@"Used cached response when we shouldn't have");
	...
	XCTAssertTrue(success,@"Cached response should not have been available");	

	// Test read from the cache
	request = [ASIHTTPRequest requestWithURL:[NSURL URLWithString:@"http://allseeing-i.com/ASIHTTPRequest/tests/cache-away"]];
	[request startSynchronous];
	success = [request didUseCachedResponse];
	XCTAssertTrue(success,@"Failed to use cached response");
	...
	[request startSynchronous];
	success = ![request didUseCachedResponse];
	XCTAssertTrue(success,@"Used the cache when reads were not enabled");
	...
	[request startSynchronous];
	success = ![request didUseCachedResponse];
	XCTAssertTrue(success,@"Used cached response when the cache should have been empty");
	...
	XCTAssertTrue(success,@"Failed to read cached response");

	success = [[[request responseHeaders] valueForKey:@"test"] isEqualToString:@"test"];
	XCTAssertTrue(success,@"Failed to read cached response headers");
	...
	success = ![request didUseCachedResponse];
	XCTAssertTrue(success,@"Request says it used a cached response, but there wasn't one to use");

	success = ([request error] != nil);
	XCTAssertTrue(success,@"Request had no error set");
	...
	success = (path != nil);
	XCTAssertTrue(success,@"Cache failed to store data");
	...
	success = (path == nil);
	XCTAssertTrue(success,@"Cache failed to remove data");
	...
	XCTAssertTrue(success,@"Request had an error");
	success = ![request contentLength];
	XCTAssertTrue(success,@"Request had a response");
}
...
- (void)testDefaultPolicy
{
	...
	XCTAssertTrue(success,@"Failed to use the cache policy from the cache");
...
- (void)testNoCache
{
	...
	for (NSString *cacheType in cacheHeaders) {
		...
		[request startSynchronous];
		BOOL success = ![request didUseCachedResponse];
		XCTAssertTrue(success,@"Data should not have been stored in the cache");
	}
}
...
- (void)testSharedCache
{
	...
	[request startSynchronous];
	BOOL success = [request didUseCachedResponse];
	XCTAssertTrue(success,@"Failed to use data cached in default cache");
	
	[ASIHTTPRequest setDefaultCache:nil];
	request = [ASIHTTPRequest requestWithURL:[NSURL URLWithString:@"http://allseeing-i.com/ASIHTTPRequest/tests/the_great_american_novel_(abridged).txt"]];
	[request startSynchronous];
	success = ![request didUseCachedResponse];
	XCTAssertTrue(success,@"Should not have used data cached in default cache");
}
...
- (void)testExpiry
{
	...
	for (NSString *header in headers) {
		...
		[request startSynchronous];
		BOOL success = ![request didUseCachedResponse];
		XCTAssertTrue(success,@"Cached data should have expired");
	}
}
...
- (void)testExtensionHandling
{
	...
	for (NSString *extension in extensions) {
		...
		XCTAssertTrue(success, @"Failed to use html extension on cached path for a resource we know a webview won't be able to open locally");
	}
	...
	NSString *path = [[ASIDownloadCache sharedCache] pathToStoreCachedResponseDataForRequest:request];
	BOOL success = [[path pathExtension] isEqualToString:@"html"];
	XCTAssertTrue(success, @"Failed to use html extension on cached path for a url without an extension");
	...
	XCTAssertTrue(success, @"Failed to preserve file extension on cached path");
}
...
- (void)testCustomExpiry
{
	...
	success = [request didUseCachedResponse];
	XCTAssertTrue(success,@"Cached data should have been used");
}
...
- (void)test304
{
	...
	XCTAssertTrue(success,@"Failed to perform a conditional get");

	success = [request didUseCachedResponse];
	XCTAssertTrue(success,@"Cached data should have been used");

	success = ([[request responseData] length]);
	XCTAssertTrue(success,@"Response was empty");
	...
	XCTAssertTrue(success, @"Failed to update expiry timestamp on 304");
}
...
- (void)testStringEncoding
{
	...
	XCTAssertTrue(success,@"Got the wrong encoding back, cannot proceed with test");

	request = [[[ASIHTTPRequest alloc] initWithURL:url] autorelease];
	[request startSynchronous];
	success = [request responseEncoding] == NSUnicodeStringEncoding;
	XCTAssertTrue(success,@"Failed to set the correct encoding on the cached response");
...
- (void)testCookies
{
	...
	XCTAssertTrue(success,@"Got no cookies back, cannot proceed with test");
	...
	for (i=0; i<[cookies count]; i++) {
		if (![[[cookies objectAtIndex:i] name] isEqualToString:[[[request responseCookies] objectAtIndex:i] name]]) {
			XCTAssertTrue(success,@"Failed to set response cookies correctly");
			return;
		}
	}
...
- (void)runCacheOnlyCallsRequestFinishedOnceTest
{
	...
	XCTAssertTrue(success,@"didFinishSelector called more than once");
}
...
- (void)runRedirectTest
{
	...
	XCTAssertTrue(success,@"Request did not redirect correctly, cannot proceed with test");
	...
	success = ([request didUseCachedResponse]);
	XCTAssertTrue(success,@"Failed to cache final response");

	XCTAssertTrue(requestRedirectedWasCalled,@"Failed to call requestRedirected");
}
...
- (void)request:(ASIHTTPRequest *)request willRedirectToURL:(NSURL *)newURL
{
	...
	success = ([request didUseCachedResponse]);
	XCTAssertTrue(success,@"Failed to cache redirect response");
...
- (void)testCachedFileOverwritten
{
	...
	XCTAssertTrue(success, @"Failed to overwrite response in cache");
}
