...
- (void)testAuthenticationHeaderGeneration
{
	...
	XCTAssertTrue(success,@"Failed to generate the correct authorisation header for a GET service request");	
	...
	XCTAssertTrue(success,@"Failed to generate the correct authorisation header for a GET request");
	...
	XCTAssertTrue(success,@"Failed to generate the correct authorisation header for a PUT request");	
	...
	XCTAssertTrue(success,@"Failed to generate the correct authorisation header for a list request");
...
- (void)testFailure
{
	...
	XCTAssertNotNil([request error],@"Failed to generate an error when the request was not correctly signed");
	
	BOOL success = ([[request error] code] == ASIS3ResponseErrorType);
	XCTAssertTrue(success,@"Generated error had the wrong error code");	
	...
	XCTAssertTrue(success,@"Generated error had the wrong description");	
...
- (void)createTestBucket
{
	...
	XCTAssertNil([bucketRequest error],@"Failed to create a bucket");		
}
...
- (void)testREST
{
	...
	XCTAssertTrue(success,@"You need to supply your S3 access details to run the REST test (see the top of ASIS3RequestTests.m)");
	...
	XCTAssertNil([serviceRequest error],@"Failed to fetch the list of buckets from S3");
	...
	XCTAssertTrue(foundBucket,@"Failed to retrive the newly-created bucket in a list of buckets");
	...
	XCTAssertTrue(success,@"Failed to PUT a file to S3");	
	...
	XCTAssertTrue(success,@"Failed to GET the correct data from S3");
	...
	XCTAssertTrue(success,@"Failed to GET a subresource");
	...
	[request setAccessKey:accessKey];
	[request startSynchronous];
	XCTAssertNil([request error],@"Failed to COPY a file");
	...
	[request startSynchronous];
	success = [[request responseString] isEqualToString:@""];
	XCTAssertTrue(success,@"Got a response body for a HEAD request");
	...
	XCTAssertNil([listRequest error],@"Failed to download a list from S3");
	success = [[listRequest objects] count];
	XCTAssertTrue(success,@"The file didn't show up in the list");	
	...
	[request startSynchronous];
	success = [[request responseString] isEqualToString:@""];
	XCTAssertTrue(success,@"Failed to DELETE the file from S3");	
	...
	[request startSynchronous];
	success = [[request responseString] isEqualToString:@""];
	XCTAssertTrue(success,@"Failed to DELETE the copy from S3");	
	...
	[request setAccessKey:accessKey];
	[request startSynchronous];
	XCTAssertNotNil([request error],@"Failed generate an error for what should have been a failed COPY");
	
	success = [[[request error] localizedDescription] isEqualToString:@"The specified key does not exist."];
	XCTAssertTrue(success, @"Got the wrong error message");	
	...
	[request startSynchronous];
	success = [[request responseString] isEqualToString:@""];
	XCTAssertTrue(success,@"Failed to PUT data to S3");
	...
	[bucketRequest setAccessKey:accessKey];
	[bucketRequest startSynchronous];
	XCTAssertNil([bucketRequest error],@"Failed to delete a bucket");
...
- (void)testGZippedContent
{
	...
	BOOL success = (![secretAccessKey isEqualToString:@""] && ![accessKey isEqualToString:@""] && ![bucket isEqualToString:@""]);
	XCTAssertTrue(success,@"You need to supply your S3 access details to run the gzipped put test (see the top of ASIS3RequestTests.m)");
	...
	[request startSynchronous];
	success = [[request responseString] isEqualToString:@""];
	XCTAssertTrue(success,@"Failed to PUT the gzipped file");		
...
{	
	...
	BOOL success = (![secretAccessKey isEqualToString:@""] && ![accessKey isEqualToString:@""] && ![bucket isEqualToString:@""]);
	XCTAssertTrue(success,@"You need to supply your S3 access details to run the list test (see the top of ASIS3RequestTests.m)");
	...
	for (i=0; i<5; i++) {
		...
		[request setAccessKey:accessKey];
		[request startSynchronous];
		XCTAssertNil([request error],@"Give up on list request test - failed to upload a file");	
	}
	...
	XCTAssertTrue(success,@"Failed to obtain a list of common prefixes");
	...
	[listRequest startSynchronous];
	XCTAssertTrue([listRequest isTruncated],@"Failed to identify what should be a truncated list of results");
	...
	XCTAssertTrue(success,@"Generated the wrong url when requesting a subresource");
	...
	XCTAssertTrue(success,@"List did not contain all files");
	...
	XCTAssertTrue(success,@"List contained files that should have been deleted");
...
- (void)GETRequestDone:(ASIS3Request *)request
{
	...
	XCTAssertTrue(success,@"Got the wrong content when downloading one of the files");
	
}

- (void)GETRequestFailed:(ASIS3Request *)request
{
	XCTAssertTrue(NO,@"GET request failed for one of the items in the list");
}
...
{
	XCTAssertTrue(NO,@"PUT request failed for one of the items in the list");
}
...
{
	XCTAssertTrue(NO,@"DELETE request failed for one of the items in the list");
}
...
//	XCTAssertTrue(success,@"Failed to update progress properly");
}
...
- (void)testCopy
{
	...
	XCTAssertTrue(success,@"Copy is of wrong class");
	...
	XCTAssertNotNil(request4,@"Failed to create a copy");
	...
	XCTAssertNotNil(bucketObject2,@"Failed to create a copy");
...
- (void)testHTTPS
{
	...
	[request startSynchronous];
	XCTAssertNil([request error],@"Failed to create a bucket");
	...
	XCTAssertTrue(success,@"Failed to PUT some data into S3");
	...
	[request startSynchronous];
	success = [[request responseString] isEqualToString:@""];
	XCTAssertTrue(success,@"Failed to DELETE the object from S3");

	// Delete the bucket
	request = [ASIS3BucketRequest DELETERequestWithBucket:bucket];
	[request setRequestScheme:ASIS3RequestSchemeHTTPS];
	[request startSynchronous];
	XCTAssertNil([request error],@"Failed to delete a bucket");
...
- (void)testCannedACLs
{
	...
	for (NSString *cannedACL in ACLs) {
		...
		[request startSynchronous];
		XCTAssertNil([request error],@"Failed to PUT some data into S3");

		// GET object ACL
		request = [ASIS3ObjectRequest requestWithBucket:bucket key:key subResource:@"acl"];
		[request startSynchronous];
		XCTAssertNil([request error],@"Failed to fetch the object");
	}
