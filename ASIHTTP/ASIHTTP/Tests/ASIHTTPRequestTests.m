...
- (void)testBasicDownload
{
	...
	XCTAssertNotNil(html,@"Basic synchronous request failed");
	...
	XCTAssertTrue(success,@"Failed to populate response headers");
	...
	XCTAssertTrue(success,@"Didn't get correct status code");	
	
	// Check data is as expected
	NSRange notFound = NSMakeRange(NSNotFound, 0);
	success = !NSEqualRanges([html rangeOfString:@"All-Seeing Interactive"],notFound);
	XCTAssertTrue(success,@"Failed to download the correct data");
	...
	XCTAssertTrue(success,@"Failed to generate an error for a bad host");
...
- (void)testBase64Encode
{
	...
	XCTAssertTrue(success,@"Failed to encode data using base64 data correctly");
}
...
- (void)runCancelTest
{
	...
	XCTAssertNotNil([request error],@"Failed to cancel the request");
	...
	XCTAssertTrue(success, @"Request did not redirect quickly enough, cannot proceed with test");
	
	XCTAssertNotNil([request error],@"Failed to cancel the request");	 
	
	success = [request totalBytesRead] < 7900198;
	XCTAssertTrue(success, @"Downloaded the whole of the response even though we should have cancelled by now");
...
- (void)performDelegateMethodsTest
{
	...
	XCTAssertTrue(started,@"Failed to call the delegate method when the request started");	
	XCTAssertTrue(receivedResponseHeaders,@"Failed to call the delegate method when the request started");	
	XCTAssertTrue(finished,@"Failed to call the delegate method when the request finished");
	...
	XCTAssertTrue(failed,@"Failed to call the delegate method when the request failed");
...
{
	XCTAssertNotNil(newResponseHeaders,@"Called request:didReceiveResponseHeaders: when we have no headers");
...
- (void)delegateTestResponseHeaders:(ASIHTTPRequest *)request
{
	XCTAssertNotNil([request responseHeaders],@"Called delegateTestResponseHeaders: when we have no headers");
	receivedResponseHeaders = YES;
}
...
- (void)testConditionalGET
{
	...
	XCTAssertTrue(success, @"Failed to download file, cannot proceed with this test");
	success = ([[request responseData] length] > 0);
	XCTAssertTrue(success, @"Response length is 0, this shouldn't happen");
	...
	XCTAssertNotNil(etag, @"Response didn't return an etag");
	XCTAssertNotNil(lastModified, @"Response didn't return a last modified date");
	...
	XCTAssertTrue(success, @"Got wrong status code");
	success = ([[request responseData] length] == 0);
	XCTAssertTrue(success, @"Response length is not 0, this shouldn't happen");
...
- (void)testException
{
	...
	XCTAssertNotNil(error,@"Failed to generate an error for an exception");
	
	BOOL success = [[[error userInfo] objectForKey:NSLocalizedDescriptionKey] isEqualToString:@"Test Exception"];
	XCTAssertTrue(success, @"Generated wrong error for exception");
...
- (void)testCharacterEncoding
{
	...
	for (i=0; i<[IANAEncodings count]; i++) {
		...
		XCTAssertTrue(success,[NSString stringWithFormat:@"Failed to use the correct text encoding for %@i",[IANAEncodings objectAtIndex:i]]);
	}
	...
	XCTAssertTrue(success,[NSString stringWithFormat:@"Failed to use the default string encoding"]);
	...
	XCTAssertTrue(success,[NSString stringWithFormat:@"Failed to parse the content type header correctly"]);
}
...
- (void)testTimeOut
{
	...
	XCTAssertTrue(success,@"Timeout didn't generate the correct error");
	...
	XCTAssertTrue(success,@"Failed to change the default timeout");	
...
- (void)testTimeOutWithoutDownloadDelegate
{
	...
	XCTAssertNil([request error],@"Generated an error (most likely a timeout) - this test might fail on high latency connections");	
}
...
- (void)testRequestMethod
{
	...
	for (NSString *method in methods) {
		...
		XCTAssertTrue(success,@"Failed to set the request method correctly");	
	}
	...
	XCTAssertTrue(success,@"Erroneously changed request method");	
}
...
- (void)testHTTPVersion
{
	...
	XCTAssertTrue(success,@"Wrong HTTP version used (May fail when using a proxy that changes the HTTP version!)");
...
- (void)testUserAgent
{
	...
	if ([ASIHTTPRequest defaultUserAgentString]) {
		...
		XCTAssertTrue(success,@"Failed to set the correct user agent");
	}
	...
	XCTAssertTrue(success,@"Failed to set the correct user-agent for a single request");
	...
	[request startSynchronous];
	success = ![[request responseString] isEqualToString:customUserAgent];
	XCTAssertTrue(success,@"Re-used a user agent when we shouldn't have done so");
	...
	[request startSynchronous];
	success = [[request responseString] isEqualToString:customUserAgent];
	XCTAssertTrue(success,@"Failed to set the correct user-agent when using a custom default");

	[ASIHTTPRequest setDefaultUserAgentString:nil];
	request = [ASIHTTPRequest requestWithURL:[NSURL URLWithString:@"http://allseeing-i.com/ASIHTTPRequest/tests/user-agent"]];
	[request startSynchronous];
	success = ![[request responseString] isEqualToString:customUserAgent];
	XCTAssertTrue(success,@"Failed to clear a custom default user-agent");
}
...
- (void)testAutomaticRedirection
{
	...
	for (i=301; i<308; i++) {
		...
		XCTAssertTrue(success,@"Got the wrong content when not redirecting after a %d",i);
		...
		XCTAssertTrue(success,@"Got the wrong content when redirecting after a %d",i);
	
		success = ([request2 responseStatusCode] == 200);
		XCTAssertTrue(success,@"Got the wrong status code (expected 200)");
	...
	for (i=301; i<303; i++) {
		...
		if (i == 303) {
			...
			XCTAssertTrue(success,@"Failed to reset request to GET on 303 redirect");
			
			success = [[request2 responseString] isEqualToString:[NSString stringWithFormat:@"Redirected as GET after a %hi status code",i]];
			XCTAssertTrue(success,@"Failed to dump the post body on 303 redirect");
			
		} else {
			success = ([request2 postLength] > 0 || ![request2 postBody] || ![[request2 requestMethod] isEqualToString:@"POST"]);
			XCTAssertTrue(success,@"Failed to use the same request method and body for a redirect when using rfc2616 behaviour");
			...
			XCTAssertTrue(success,@"Failed to send the correct post body on redirect");
		}
	}
	...
	XCTAssertTrue(success,@"Failed to store just the body of the file request on redirect");
	
	success = ([request originalURL] != [request url]);
	XCTAssertTrue(success,@"Failed to update request url on redirection");
	
	success = ([[[request originalURL] absoluteString] isEqualToString:@"http://allseeing-i.com/ASIHTTPRequest/tests/redirect/301"]);
	XCTAssertTrue(success,@"Failed to preserve original url");	
	...
	XCTAssertTrue(success,@"Failed to preserve original user agent on redirect");
}
...
- (void)testResumeChecksContentRangeHeader
{
	...
	XCTAssertTrue(success,@"Downloaded whole file too quickly, cannot proceed with this test");
	...
	XCTAssertTrue(success,@"Failed to obtain correct partial dowload size");
	[request startAsynchronous];
	...
	XCTAssertNil([request error],@"Request failed, cannot proceed with this test");

	success = (![[request responseHeaders] objectForKey:@"Content-Range"]);
	XCTAssertTrue(success,@"Got range header back, cannot proceed with this test");
	...
	XCTAssertTrue(success,@"Downloaded file has wrong length");

	success = ([request partialDownloadSize] == 0);
	XCTAssertTrue(success,@"Failed to reset download size");
}
...
- (void)runRedirectedResume
{
	...
	XCTAssertTrue(success,@"Restarted download when we should have resumed, or asked for the wrong segment of the file");
...
- (void)testUploadContentLength
{
	...
	XCTAssertTrue(success,@"Sent wrong content length");
}
...
- (void)testDownloadContentLength
{
	...
	XCTAssertTrue(success,@"Got wrong content length");
}
...
- (void)testFileDownload
{
	...
	XCTAssertNotNil(image,@"Failed to download data to a file");
}
...
- (void)testCompressedResponseDownloadToFile
{
	...
	XCTAssertNil(tempPath,@"Failed to clean up temporary download file");		
	...
	XCTAssertTrue(success,@"Failed to download data to a file");
...
- (void)testCompressedResponseDelegateDataHandling
{
	...
	XCTAssertTrue(success,@"Failed to correctly download the response using a delegate");
...
- (void)performDownloadProgressTest
{
	...
	XCTAssertTrue(success,@"Failed to properly increment download progress %f != 1.0",progress);	
	...
	XCTAssertTrue(success,@"Downloaded too quickly, cannot proceed with test");	
	 
	success = (progress > 0);
	XCTAssertTrue(success,@"Either downloaded too slowly, or progress is not being correctly updated");		 
...
- (void)performUploadProgressTest
{
	...
	BOOL success = (progress == 1.0);
	XCTAssertTrue(success,@"Failed to properly increment upload progress %f != 1.0",progress);	
}
...
- (void)performPostBodyStreamedFromDiskTest
{
	...
	XCTAssertTrue(success,@"Failed upload the correct request body");
...
- (void)testCookies
{
	...
	XCTAssertTrue(success,@"Failed to set a cookie");
	
	// Test a cookie is stored in responseCookies
	NSArray *cookies = [request responseCookies];
	XCTAssertNotNil(cookies,@"Failed to store cookie data in responseCookies");
	...
	for (cookie in cookies) {
		if ([[cookie name] isEqualToString:@"ASIHTTPRequestTestCookie"]) {
			...
			XCTAssertTrue(success,@"Failed to store the correct value for a cookie");
			success = [[cookie domain] isEqualToString:@"allseeing-i.com"];
			XCTAssertTrue(success,@"Failed to store the correct domain for a cookie");
			success = [[cookie path] isEqualToString:@"/ASIHTTPRequest/tests"];
			XCTAssertTrue(success,@"Failed to store the correct path for a cookie");
			break;
		}
	}
	XCTAssertTrue(foundCookie,@"Failed store a particular cookie - can't continue with the rest of the tests");
	...
	XCTAssertTrue(success,@"Cookie not presented to the server with cookie persistence OFF");
	...
	html = [request responseString];
	success = [html isEqualToString:@"I have 'This is the value' as the value of 'ASIHTTPRequestTestCookie'"];
	XCTAssertTrue(success,@"Cookie not presented to the server with cookie persistence ON");
	...
	XCTAssertTrue(success,@"Failed to remove a cookie");
	...
	XCTAssertTrue(success,@"Cookie presented to the server when it should have been removed");
	...
	XCTAssertNotNil(cookie,@"Failed to create a cookie - cookie value was not correctly encoded?");
	...
	XCTAssertTrue(success,@"Custom cookie not presented to the server with cookie persistence OFF");
	...
	XCTAssertTrue(success,@"Cookies not removed");
...
- (void)testPreserveResponseWhenDownloadComplete
{
	...
	XCTAssertTrue(success,@"Request removed the response body when we encountered an error, even though the download was complete");
	...
	XCTAssertTrue(success,@"Request removed or failed to copy the response to downloadDestinationPath");
}
...
- (void)testBasicAuthentication
{
	...
	XCTAssertTrue(success,@"Failed to generate permission denied error with no credentials");
	...
	[request startSynchronous];
	success = [[request error] code] == ASIAuthenticationErrorType;
	XCTAssertTrue(success,@"Failed to generate permission denied error with wrong credentials");
	...
	[request startSynchronous];
	err = [request error];
	XCTAssertNil(err,@"Failed to supply correct username and password");
	...
	[request startSynchronous];
	success = [[request error] code] == ASIAuthenticationErrorType;
	XCTAssertTrue(success,@"Reused credentials when we shouldn't have");
	...
	[request startSynchronous];
	err = [request error];
	XCTAssertNil(err,@"Failed to reuse credentials");
	...
	[request startSynchronous];
	success = [[request error] code] == ASIAuthenticationErrorType;
	XCTAssertTrue(success,@"Failed to clear credentials");
	...
	[request startSynchronous];
	err = [request error];
	XCTAssertNil(err,@"Failed to use stored credentials");
	...
	XCTAssertTrue(success,@"Didn't supply credentials before being asked for them when talking to the same server with shouldPresentCredentialsBeforeChallenge == YES");
	...
	XCTAssertTrue(success,@"Supplied session credentials before being asked for them");	
	...
	XCTAssertFalse(fail,@"Sent Basic credentials even though request did not have kCFHTTPAuthenticationSchemeBasic set as authenticationScheme.");
	...
	[request startSynchronous];
	success = [request authenticationRetryCount] == 0;
	XCTAssertTrue(success,@"Didn't supply credentials before being asked for them, even though they were set on the request and shouldPresentCredentialsBeforeChallenge == YES");
	...
	[request startSynchronous];
	success = [request authenticationRetryCount] == 1;
	XCTAssertTrue(success,@"Supplied request credentials before being asked for them");	
...
- (void)testDigestAuthentication
{
	...
	[request startSynchronous];
	success = [request authenticationRetryCount] == 0;
	XCTAssertTrue(success,@"Didn't supply credentials before being asked for them, even though they were set in the session and shouldPresentCredentialsBeforeChallenge == YES");	
...
- (void)testNTLMHandshake
{
	...
	XCTAssertNil([request error],@"Got an error when credentials were supplied");
	...
	XCTAssertTrue(success,@"Failed to send credentials correctly? (Expected: '%@', got '%@')",expectedResponse,[[request responseString] lowercaseString]);
}
...
- (void)testCompressedResponse
{
	...
	XCTAssertTrue(success,@"Got incorrect request headers from server");

	success = ([request rawResponseData] == [request responseData]);
	XCTAssertTrue(success,@"Attempted to uncompress data that was not compressed");	

	url = [[[NSURL alloc] initWithString:@"http://allseeing-i.com/ASIHTTPRequest/tests/first"] autorelease];
	request = [[[ASIHTTPRequest alloc] initWithURL:url] autorelease];
	[request startSynchronous];
	success = ([request rawResponseData] != [request responseData]);
	XCTAssertTrue(success,@"Uncompressed data is the same as compressed data");	

	success = [[request responseString] isEqualToString:@"This is the expected content for the first string"];
	XCTAssertTrue(success,@"Failed to decompress data correctly?");
	...
	[request startSynchronous];
	success = ([request rawResponseData] == [request responseData]);
	XCTAssertTrue(success,@"Failed to populate rawResponseData with the inflated data");
}
...
- (void)performPartialFetchTest
{
	...
	XCTAssertTrue(success,@"Failed to download a segment of the data");
	...
	XCTAssertTrue(success,@"Failed to append the correct data to the end of the file?");
	
	success = (progress == 1.0);
	XCTAssertTrue(success,@"Failed to correctly display increment progress for a partial download");
}
...
- (void)test000SSL
{
	...
	XCTAssertNotNil([request error],@"Failed to generate an error for a self-signed certificate (Will fail on the second run in the same session!)");		
	...
	XCTAssertTrue(success,@"Generated the wrong error for a self signed cert");
	...
	XCTAssertNil([request error],@"Failed to accept a self-signed certificate");	
}
...
- (void)testRedirectPreservesSession
{
	...
	XCTAssertTrue(success,@"Failed to redirect preserving session cookies");	
}
...
- (void)testTooMuchRedirection
{
	...
	[request startSynchronous];
	XCTAssertNotNil([request error],@"Failed to generate an error when redirection occurs too many times");
	BOOL success = ([[request error] code] == ASITooMuchRedirectionErrorType);
	XCTAssertTrue(success,@"Generated the wrong error for a redirection loop");		
}
...
- (void)testRedirectToNewDomain
{
	...
	XCTAssertTrue(success,@"Failed to redirect to a different domain");		
}
...
- (void)test303Redirect
{
	...
	XCTAssertTrue(success,@"Failed to redirect to correct location");
	success = [[request responseString] isEqualToString:@"GET"];
	XCTAssertTrue(success,@"Failed to use GET on new URL");
}
...
- (void)testCompressedBody
{
	...
	XCTAssertTrue(success,@"Failed to compress the body, or server failed to decompress it");
...
- (void)testSubclass
{
	...
	XCTAssertTrue(success,@"Convenience constructor failed to return an instance of the correct class");	
}
...
- (void)testThrottlingDownloadBandwidth
{
	...
	XCTAssertTrue(success,@"Downloaded the file too slowly - either this is a bug, or your internet connection is too slow to run this test (must be able to download 128KB in less than 7 seconds, without throttling)");
	...
	success = (interval < -7);
	XCTAssertTrue(success,@"Failed to throttle download");		
	XCTAssertNil([request error],@"Request generated an error - timeout?");	
...
- (void)testThrottlingUploadBandwidth
{
	...
	XCTAssertTrue(success,@"Uploaded the data too slowly - either this is a bug, or your internet connection is too slow to run this test (must be able to upload 64KB in less than 3 seconds, without throttling)");
	...
	interval =[date timeIntervalSinceNow];
	success = (interval < -3);
	XCTAssertTrue(success,@"Failed to throttle upload");		
	XCTAssertNil([request error],@"Request generated an error - timeout?");	
}
...
- (void)testFetchToInvalidPath
{
	...
	[request startSynchronous];
	XCTAssertNotNil([request error],@"Failed to generate an authentication when attempting to write to an invalid location");	
...
- (void)testResponseStatusMessage
{
	...
	XCTAssertTrue(success,@"Got wrong response status message");
}
...
- (void)asyncFail:(ASIHTTPRequest *)request
{
	...
	XCTAssertTrue(success,@"Wrong request failed");
}

- (void)asyncSuccess:(ASIHTTPRequest *)request
{
	BOOL success = ([request tag] != 4);
	XCTAssertTrue(success,@"Request succeeded when it should have failed");
	...
	XCTAssertTrue(success,@"Got wrong request content - very bad!");
...
- (void)testAutomaticRetry
{
	...
	[request startSynchronous];
	XCTAssertNotNil([request error],@"Request failed to timeout, cannot proceed with test");
	BOOL success = ([request retryCount] == 5);
	XCTAssertTrue(success,@"Request failed to retry on timeout");
}
...
- (void)testCopy
{
	...
	XCTAssertNotNil(request2,@"Failed to create a copy");
	BOOL success = ([request2 retainCount] == 1);
	XCTAssertTrue(success,@"Failed to create a retained copy");
...
- (void)testCloseConnection
{
	...
	XCTAssertTrue(success,@"Should not be able to re-use a request sent with Connection:close");
...
- (void)testPersistentConnections
{
	...
	XCTAssertTrue(success,@"Request failed to default to 60 seconds for connection timeout");
	...
	XCTAssertTrue(success,@"Request failed to use time out set by server");
	...
	XCTAssertTrue(success,@"Reused a connection that should have timed out");
	...
	XCTAssertTrue(success,@"Used a persistent connection with a body");
	...
	success = ![request shouldAttemptPersistentConnection];
	XCTAssertTrue(success,@"Used a persistent connection with PUT");
	...
	success = ![request shouldAttemptPersistentConnection];
	XCTAssertTrue(success,@"Used a persistent connection with POST");
	...
	success = [request shouldAttemptPersistentConnection];
	XCTAssertTrue(success,@"Failed to use a persistent connection");
}
...
- (void)runRemoveUploadProgressTest
{
	...
	BOOL success = (progress == 1.0);
	XCTAssertTrue(success,@"Failed to set upload progress, cannot proceed with test");
...
- (void)testMimeType
{
	...
	XCTAssertTrue(success,@"Failed to detect the mime type for a file");
	
	filePath = @"/nowhere";
	success = (![ASIHTTPRequest mimeTypeForFileAtPath:filePath]);
	XCTAssertTrue(success,@"Returned a mime type for a non-existent file");
	...
	XCTAssertTrue(success,@"Failed to return the default mime type when a file has no extension");
}
...
- (void)theTestRequestFinished:(ASIHTTPRequest *)request
{
	...
	XCTAssertTrue(success,@"Failed to correctly download and store the response using a delegate");
}
...
- (void)testRFC1123DateParsing
{
	...
	XCTAssertTrue(success,@"Failed to parse an RFC1123 date correctly");
...
- (void)testAccurateProgressFallback
{
	...
	XCTAssertTrue(success,@"Request failed to fall back to simple progress");
	...
	XCTAssertTrue(success,@"Request fell back to simple progress when redirecting");
}
...
- (void)testResumeWithAutomaticTimeoutRetry
{
	...
	XCTAssertTrue(success, @"Failed to send the correct Range headers to the server when resuming after a timeout");
}
...
- (void)redirectURLTestSucceeded:(ASIHTTPRequest *)request
{
	...
	XCTAssertTrue(success,@"Request failed to redirect to url specified by delegate");
}
