...
- (void)runDelegateMethodsTest
{
	...
	XCTAssertTrue(receivedResponseHeaders,@"Failed to call the delegate method when the request received response headers");
...
{
	XCTAssertNotNil(responseHeaders,@"Called delegateTestResponseHeaders: when we have no headers");
...
- (void)testDownloadProgress
{
	...
	XCTAssertTrue(success,@"Failed to increment progress properly");
...
- (void)addMoreRequestsQueueFinished:(ASINetworkQueue *)queue
{
	...
	XCTAssertTrue(success,@"Failed to increment total download size properly");	
}
...
- (void)testFailure
{
	...
	XCTAssertTrue(success,@"Request 1 failed");
	
	success = [[request1 responseString] isEqualToString:@"This is the expected content for the first string"];
	XCTAssertTrue(success,@"Failed to download the correct data for request 1");
	
	success = ([request2 error] == nil);
	XCTAssertTrue(success,@"Request 2 failed");
	
	success = [[request2 responseString] isEqualToString:@"This is the expected content for the second string"];
	XCTAssertTrue(success,@"Failed to download the correct data for request 2");
	
	success = ([request3 error] == nil);
	XCTAssertTrue(success,@"Request 3 failed");
	
	success = [[request3 responseString] isEqualToString:@"This is the expected content for the third string"];
	XCTAssertTrue(success,@"Failed to download the correct data for request 3");
	
	success = ([requestThatShouldFail error] != nil);
	XCTAssertTrue(success,@"Request 4 succeed when it should have failed");
	
	success = ([request5 error] == nil);
	XCTAssertTrue(success,@"Request 5 failed");
	
	success = ([request5 responseStatusCode] == 404);
	XCTAssertTrue(success,@"Failed to obtain the correct status code for request 5");
...
- (void)testProgressWithAuthentication
{
	...
	XCTAssertNotNil(error,@"The HEAD request failed, but it didn't tell the main request to fail");	
	...
	error = [request error];
	XCTAssertNil(error,@"Failed to use authentication in a queue");	
...
- (void)authenticationNeededForRequest:(ASIHTTPRequest *)request
{
	...
	if ([[[request userInfo] objectForKey:@"test"] isEqualToString:@"ntlm"]) {
		...
		XCTAssertTrue(success,@"Delegate was asked for credentials more than once");
...
- (void)testWithNoListener
{
	...
	XCTAssertTrue(!request_succeeded && request_didfail,@"Request to resource without listener succeeded but should have failed (May fail with proxy!)");
...
- (void)testPartialResume
{
	...
	XCTAssertNil(err,@"Got an error obtaining attributes on the file, this shouldn't happen");
	
	BOOL success = (downloadedSoFar > 0);
	XCTAssertTrue(success,@"Failed to download part of the file, so we can't proceed with this test");
	...
	XCTAssertTrue(success,@"Failed to complete the download");
	...
	XCTAssertTrue(success,@"Temporary download file doesn't exist");	
	
	[request removeTemporaryDownloadFile];
	
	success = (![[NSFileManager defaultManager] fileExistsAtPath:temporaryPath]);
	XCTAssertTrue(success,@"Temporary download file should have been deleted");		
...
- (void)testMultipleDownloadsThrottlingBandwidth
{
	...
	XCTAssertTrue(success,@"Downloaded the data too slowly - either this is a bug, or your internet connection is too slow to run this test (must be able to download 90KB in less than 6 seconds, without throttling)");
...
- (void)testMultipleUploadsThrottlingBandwidth
{
	...
	XCTAssertTrue(success,@"Uploaded the data too slowly - either this is a bug, or your internet connection is too slow to run this test (must be able to upload 160KB in less than 10 seconds, without throttling)");
...
- (void)throttleFail:(ASIHTTPRequest *)request
{
	XCTAssertTrue(NO,@"Request failed, cannot continue with this test");
...
- (void)postDone:(ASIHTTPRequest *)request
{
	...
	XCTAssertTrue(success,@"Didn't post correct data");	
}
...
- (void)runHEADFailureTest
{
	...
	XCTAssertTrue(headFailed,@"Failed to notify the request's delegate");
}
...
- (void)testCopy
{
	...
	XCTAssertNotNil(queue2,@"Failed to create a copy");
...
- (void)runTestQueueFinishedCalledOnFailureTest
{
	...
	XCTAssertTrue(complete,@"Failed to call queue finished delegate method");
	...
	XCTAssertTrue(success,@"Called the queue finish method more/less than once");
