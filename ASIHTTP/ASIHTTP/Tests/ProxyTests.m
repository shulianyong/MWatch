...
- (void)testProxyForHTTPS
{
	...
	XCTAssertTrue(success,@"Failed to connect to an HTTPS URL using a proxy");
}
...
- (void)testAutoConfigureWithPAC
{
	...
	[request startSynchronous];
	XCTAssertNil([request proxyHost],@"Shouldn't use a proxy here");
	XCTAssertNil([request error],@"Request failed when unable to fetch PAC (should assume no proxy instead)");
	...
	XCTAssertTrue(success,@"Failed to use the correct proxy");
	
	request = [ASIHTTPRequest requestWithURL:[NSURL URLWithString:@"http://www.apple.com"]];
	[request setPACurl:[NSURL URLWithString:pacurl]];
	[request startSynchronous];
	XCTAssertNil([request proxyHost],@"Used a proxy when the script told us to go direct");
}
...
- (void)testProxy
{
	...
	XCTAssertTrue(success,@"You need to supply the details of your proxy to run the proxy autodetect test");	
	...
	XCTAssertTrue(success,@"Failed to download the correct data, navigating the proxy");
}
...
{	
	...
	XCTAssertTrue(success,@"Failed to detect the proxy");		
}
...
- (void)testProxyWithSuppliedAuthenticationCredentials
{
	...
	XCTAssertTrue(success,@"You need to supply the details of your authenticating proxy to run the proxy authentication test");	
...
{
	[self setComplete:YES];
	XCTAssertTrue(0,@"Request failed when it shouldn't have done so");	
}
